...
if not ishave:
	cursor.execute("insert into base_1 (`num`,`time`,`today`,`type`) values (%s, %s,%s,%s)",[dau,sec,today,'dau'])
else:
	cursor.execute("update base_1 set `num`=%s where `type` = 'dau' and `time` = %s",[dau,sec])
cursor.close()
conn.commit()
conn.close()